...
class Evolution:
    ...
    def match(self, games):
        for i in range(len(self.players)):
            result = dual(self.players[i], Player(), games)
            self.results[i] = [self.results[i][j] + result[j] for j in range(len(result))]
    ...
    def best(self, games):
        results = []
        result = dual(self.players[0], Player(), games)
        results.append(round((result[1] - result[2])/float(games),2))
        return results
...
for i in range(10000):
    print "Scoring..."
    print "Generation {}:".format(i), sum(e.best(100)), sum(e.best(100)), sum(e.best(100)), sum(e.score(10)), e.players[0].name
    e.generation(100)
